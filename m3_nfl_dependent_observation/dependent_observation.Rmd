...
grid_epsilon <- seq(0.01, 1, 0.02)

simple_sim_posterior_ll <- main_simulation_uc(subject = 1,
                simple_stimuli %>% filter(trial_number < 3), 
...
for (t in seq(1: length(simple_sim_posterior_ll))){
  for (f in seq(1:length(simple_sim_posterior_ll[[t]]))){
    simple_sim_posterior_ll[[t]][[f]]$t <- t  
  }
}

bind_rows(simple_sim_posterior_ll) %>% 
  filter(!is.na(posterior)) %>% 
  ggplot(aes(x  = theta, y = posterior, color = epsilon)) + 
  geom_point() + 
  facet_wrap(~t)
  #facet_grid(t~epsilon)
...
sim_res <- main_simulations(subject = 3,
                simple_stimuli, 
...
grid_theta <- seq(0.01, 1, 0.02)
grid_epsilon <- seq(0.01, 1, 0.02)
res <- main_simulation_testing(subject = 1,
                simple_stimuli, 
                noise_parameter = 0.1, 
                eig_from_world = env_eig/10,
                max_observation = max_obs, # should this be per trial or in total? currently per trial 
...
```{r}
plot_kl_if_true <- res[[1]] %>% 
  filter(stimulus_idx == 1) %>% 
  ggplot(aes(x = t, y = kl_if_true$kl_if_true, color = V1)) + 
  geom_point() + 
  theme(legend.position = "none") +
  labs(subtitle = "kl when hypothetical obs is TRUE")

plot_pp_if_true <- res[[1]] %>% 
  filter(stimulus_idx == 1) %>% 
  ggplot(aes(x = t, y = pp_if_true$pp_if_true, color = V1)) + 
  geom_point() + 
    theme(legend.position = "none") +
...
plot_eig_from_both_sides <- res[[1]] %>% 
  mutate(eig_true_side = kl_if_true$kl_if_true * pp_if_true$pp_if_true, 
         eig_false_side = kl_if_false$kl_if_false * pp_if_false$pp_if_false, 
...
plot_kl_if_false <- res[[1]] %>% 
  filter(stimulus_idx == 1) %>% 
  ggplot(aes(x = t, y = kl_if_false$kl_if_false, color = V1)) + 
  geom_point() + 
    theme(legend.position = "none") +

  labs(subtitle = "kl when hypothetical obs is FALSE")

plot_pp_if_false <- res[[1]] %>% 
  filter(stimulus_idx == 1) %>% 
  ggplot(aes(x = t, y = pp_if_false$pp_if_false, color = V1)) + 
  geom_point() + 
    theme(legend.position = "none") +
