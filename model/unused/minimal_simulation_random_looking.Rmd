...
source(here("helper/main_simulation_helper_random_looking.R"))
...
# save RDS
saveRDS(all_sims_res %>% 
  group_by(row_number, params_id, n_features, on_features_n, sequence_scheme, stimulus_idx) %>% 
  filter(!is.na(look_away)) %>% 
  summarise(sample_n = n(), stim_info = stim_info, params_info = params_info) %>% rename(sim_id = row_number) %>% select(sim_id, stimulus_idx, sample_n, params_id, stim_info), 'random_looking_sim.RDS')
