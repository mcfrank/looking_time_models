...
dissimilar_ratio = 1
softmax_greed = 1
sequence_scheme = c("BBBBB", "BBBBBBB", "BBBBBBBBBB")
...
forced_exposure_params = tibble(do_forced_exposure = FALSE,  # FALSE = forced exposure is off; TRUE = forced exposure is on
                                forced_exposure_trials = "first", # "first" sets the first trial to a fixed exposure, "allbutlast" will set all of them to fixed exposure but the last 
                                max_sample_num = 23 # number of forced samples in forced exposure trials
                                  )  
...
ggplot(complete_sim_res, 
  ...
  stat_summary(fun.data = "mean_cl_boot", geom = "line", position = position_dodge(width = .1)) +facet_grid(alpha_prior~sequence_scheme)
