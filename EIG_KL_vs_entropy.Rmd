...
for (i in 1:length(kl_true)) {
  ...
  kl_false[i] = KL(rbind(posterior,false_obs))
    
  H_true[i] = H(true_obs) - H(posterior) 
  H_false[i] = H(false_obs) - H(posterior) 
}
...
df <- tibble(alpha = alpha, beta = beta, 
  ...
  pivot_longer(cols = c("kl_true", "kl_false", "entropy_true", "entropy_false"), 
               names_to = "metric", values_to = "value") %>% 
  separate(metric, into = c("metric", "true_or_false"))


ggplot(data = df, aes(x = beta, y = value, color = true_or_false)) + 
  geom_point() + facet_wrap(~metric) + ggtitle('fixed alpha = 100')
