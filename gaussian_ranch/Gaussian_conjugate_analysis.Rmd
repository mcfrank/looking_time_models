...
getNIG_EIG <- function(m_p, V_p, a_p, b_p){
   hypothetical_obs <- -seq(-5, 5, by = 0.01)
   hypothetical_kl <- rep(NA, length(hypothetical_obs))
   post_pred <- normal_inverse_gamma_post_pred(hypothetical_obs, m_n, V_n, a_n, b_n)
   print(post_pred)
   for (i in hypothetical_obs) {
        new_params <- nig_posterior( m_p, V_p, a_p, b_p, i)
        hypothetical_kl[i] <- normal_inverse_gamma_KL(m_p, new_params[1], 
                                                      V_p, new_params[2],
                                                      a_p, new_params[3],
                                                      b_p, new_params[4])
        #print( hypothetical_kl[i] )
   }
   EIG = post_pred %*% hypothetical_kl
   return(EIG)
}
...
im_df <- main_update_function(prior_mu = 1, prior_V = 1, prior_alpha = 1, prior_beta = 1, observations = X)
