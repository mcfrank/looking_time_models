...
# takes a df of parameters and some globals
granch_backward_im_simulation <- function(params = df, testing = TRUE) {
  ...
  if(params$im == "KL" | params$im == "surprisal"){
  ...
  #while(t < 5){   
    ...
    model$im[t] <- sum(im_all[t, ])
    
    
    # currently taking away the probabilistic decision-making process
    
    model$look_away[t] = model$im[t] < model$EIG_from_world[t]
    #model$look_away[t] = t > 5
