...
df_sim %>% 
  group_by(sim_id, n_hypo_obs, stimulus_idx) %>% 
  summarise(sample_n = n()) %>% 
  ggplot(aes(x = stimulus_idx, y = sample_n, 
             color = as.factor(sim_id))) + 
  geom_line() + 
  ylim(0, 75)  + 
  facet_wrap(~n_hypo_obs)


df_sim %>% 
  group_by(sim_id,len_grid_mu_theta, n_f, n_hypo_obs, stimulus_idx) %>% 
  summarise(sample_n = n()) %>% 
  ggplot(aes(x = stimulus_idx, y = sample_n, 
             color = as.factor(sim_id))) + 
  geom_line() + 
  ylim(0, 75)  + 
  facet_grid(n_f~len_grid_mu_theta)
```
...
df %>% 
  filter(n_hypo_obs == 3) %>% 
  filter(n_f < 6) %>% 
  ggplot(aes(x = prior_grid_row, y = ranch_sim_time, 
             color = as.factor(n_f) 
             )) + 
  geom_point() + 
  geom_line()
...
optim_res<- optim(c(10, 1, .8), optim_runtime_fit_wrapper)
```
...
```{r}
/Users/caoanjie/Desktop/projects/model/model/gaussian_ranch/exploration/exploration_cached_res/sBBBDBB_res.RDS
```


```{r}
df <- list("sBBBBBD_res.RDS",
           "sBBBDBB_res.RDS", 
           "sBDBBBB_res.RDS)") %>% 
unlist() %>% 
  sapply(., function(x) paste0(here("exploration/exploration_cached_res/"), x)) %>% 
  map(readRDS) %>% 
  bind_rows()
...
d_last_res <- readRDS(here("exploration/exploration_cached_res/sBBBBBD_res.RDS"))


d_last_res %>% 
  select(stimulus_idx, f1, f2, f3) %>% 
  distinct(stimulus_idx, .keep_all = TRUE)


d_last_res %>% group_by(stimulus_idx) %>% 
  summarise(sample_n = n()) %>% 
  ggplot(aes(x = stimulus_idx, y = sample_n)) +  
  geom_point()
```
...
d_4th_res <- readRDS(here("exploration/exploration_cached_res/sBBBDBB_res.RDS"))
...
d_4th_res %>% group_by(stimulus_idx) %>% 
  summarise(sample_n = n()) %>% 
  ggplot(aes(x = stimulus_idx, y = sample_n)) +  
  geom_point()
```
...
d_2_res <- readRDS(here("exploration/exploration_cached_res/sBDBBBB_res.RDS"))
