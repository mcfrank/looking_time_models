...
```{r}
set.seed(1)
X = rnorm(100, mean = 5, sd = 3) + rnorm(100, mean = 0, sd = 0.5)
...
# should be different, when distributions are different
...
main_update_function <- function(prior_mu, 
  ...
  prior_post_pred <- rep(NA, length(observations) + 1)
  priors <- vector(mode = "list", length = length(observations) + 1)
  posts <- vector(mode = "list", length = length(observations) + 1)
  ...
  for (i in 1:length(observations)){
    ...
    prior_post_pred[i] <- normal_inverse_gamma_post_pred(current_observation, prior_mu, prior_V, prior_alpha, prior_beta)
    
    print('prior_post_pred[i]')
    print(prior_post_pred[i])
    
    surprisal <- -log(prior_post_pred[i])
  ...
  im_df <- tibble(
    ...
    eig = eigs,
    prior = priors,
    prior_post_pred = prior_post_pred 
  )
...
plotNormal(mu_0, sqrt(1/kappa_0))
plotInvGamma(alpha_0, beta_0)
plotNormalInvGamma(mu_0, 1/kappa_0, alpha_0, beta_0)

n = 10

Y = rep(1, n) 
Z = Y + rnorm(n, mean = m_e, sd = sigma_e)
sample_mean = mean(Z)

mu_n = ((mu_0 * kappa_0) + n * sample_mean) / (kappa_0 + n)
kappa_n = kappa_0 + n 
alpha_n = alpha_0 + n/2
beta_n = beta_0 + 1/2 * sum((Z - sample_mean)^2) + (kappa_0*n*(sample_mean - mu_0) ^ 2)/(2*(kappa_0 + n))

#plotNormal(mu_n, sqrt(1/kappa_n))
#plotInvGamma(alpha_n, beta_n)
#plotNormalInvGamma(mu_n, 1/kappa_n, alpha_n, beta_n)
...
# the cumulative distribution function of the inverse gamma distribution is the upper incomplete gamma function with params (alpha, beta/x) divided by gamma(alpha)

cum_dens_inv_gamma <- function(sigma, alpha, beta) {
  
  numerator = incgam(sigma^2, beta/sigma^2)

  denominator = gamma(alpha) 

  cum_dens_inv_gamma = numerator / denominator

  
}

# compare this to the cumulative density of the normal inverse gamma, for x -> infinity, sig = sigma_eps
cum_dens_norm_inv_gamma <- function(mu_y, sig, mu_NIG, lambda, alpha, beta) {
  
  arg_erf = (sqrt(lambda)*(mu_y-mu_NIG) )/(sqrt(2)*sig)
  
  numerator  = exp(-beta/sig^2) * (beta/sig^2)^alpha * (erf(arg_erf) + 1)
  
  denominator = 2*sig^2 * gamma(alpha)
  
  cum_dens_norm_inv_gamma = numerator / denominator
  
}

# truncate the NIG distribution


# renormalize

# compute the cumulative density until e^2



# shift it back by epsilon ^2
```
