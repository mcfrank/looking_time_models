...
```{r}
fitting_res %>% rename(train_r = r, 
                       train_rmse = rmse) %>% 
  left_join(fitting_test_res %>% 
              rename(test_r = r, test_rmse = rmse), by = "param_info") %>% 
  select(param_info, train_r, test_r, train_rmse, test_rmse) %>% 
  pivot_longer(
    cols = c("train_r", "test_r", "train_rmse", "test_rmse")
  ) %>% 
  separate(
    name, into = c("data_type", "measure_type"), sep = "_"
  ) %>% 
...
```{r}
cd %>% 
  distinct(b_val, d_val, complexity) %>% 
  pivot_longer(cols  = c("b_val", "d_val")) %>% 
...
cd %>% 
  ...
  ggplot(aes(x = complexity, y = dist)) + 
  geom_jitter(alpha = .3, width = .2) + 
  theme_few()
```
