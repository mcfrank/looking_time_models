...
#need to get the block number back 

test_d <- raw_data %>% 
  filter(subject %in% unique(test_d$subject)) %>% 
  mutate(
    stimuli_sequence = case_when(
      deviant_position == 2 ~ "BDBBBB", 
      deviant_position == 4 ~ "BBBDBB", 
  ...
  select(subject, block_number, trial_number, trial_type, stimuli_sequence, complexity, trial_looking_time) 
...
saveRDS(scaling_param_f, here("behavior_fit/cached_res/scaling_f.RDS"))
...
scaling_nn <- get_scaling_param(nn_test_d)

nn_flipped_scale <- get_scaling_param_flipped(nn_test_d)
saveRDS(nn_flipped_scale, here("behavior_fit/cached_res/nn_scale_f.RDS"))

saveRDS(fitting_nn_res, here("behavior_fit/cached_res/nn_fit.RDS"))
saveRDS(scaling_nn, here("behavior_fit/cached_res/nn_scale.RDS"))
```
...
nl_scale <- get_scaling_param_constraint(nl_test_d)
nl_flipped_scale <- get_scaling_param_flipped(nl_test_d)
saveRDS(nl_res, here("behavior_fit/cached_res/nl_test_fit.RDS"))
saveRDS(nl_scale, here("behavior_fit/cached_res/nl_scale.RDS"))
saveRDS(nl_flipped_scale, here("behavior_fit/cached_res/nl_scale_f.RDS"))
```
