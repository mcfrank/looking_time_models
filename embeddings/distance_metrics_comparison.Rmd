...
ggplot(embeddings_df, aes(x = V1, y = V2, color = number)) + facet_grid(~animacy) + geom_point() + xlab("PC1") + ylab("PC2") + theme_classic(base_size = 20)
...
idx = 0
for (animal in embeddings_df %>% filter(animacy == 'animate') %>% pull(stim_name)) {
   idx = idx + 1
   ...
   # number violation
   number_violations = embeddings_df %>% filter(pose == cur_pose, animacy == cur_animacy, number != cur_number) %>% pull(stim_name)
...
long_distance_df = pivot_longer(distance_df_animals, cols = c('pose_dist', 'identity_dist', 'animacy_dist', 'number_dist'), names_to = 'violation_type') %>% mutate(violation_type = factor(violation_type, levels = c('animacy_dist', 'identity_dist', 'pose_dist', 'number_dist')))
...
for (veggie in embeddings_df %>% filter(animacy == 'inanimate') %>% pull(stim_name)) {
  ...
  distance_df_animals$stim_name[idx] = veggie
  distance_df_animals$pose_dist[idx] = pose_violation_dist
  distance_df_animals$identity_dist[idx] = identity_violation_dist
  distance_df_animals$animacy_dist[idx] = animacy_violations_dist
  distance_df_animals$number_dist[idx] = number_violations_dist
