...
# currently looking at one feature but needs to be updated later to look at different feature 

update_z_given_theta <- function(concept, current_combo, obs_t_from_concept, model, 
                         lp_y_given_theta, lp_z_given_theta){
  ...
  for (i in 1:length(current_combo)){
    # if the stimuli just seen came from the concept that we are interested in 
    # initializing p(z|y) for this particular y
    ...
    if(current_combo[stimulus_id] == concept){
      # then grab all of the observations on that stimuli that came from this concept 
      observation_on_current_stimulus <- model$f1[model$stimulus_idx == i][!is.na(model$f1[model$stimulus_idx == i])]
      ...
      this_lp_z_given_theta$lp_z_given_theta <- rowLogSumExps(lx = as.matrix(this_lp_z_given_theta[,c("lp_z_y_ONE", "lp_z_y_ZERO")]))
      
       lp_z_theta$z_theta  <-  lp_z_theta$z_theta  +  this_lp_z_given_theta$lp_z_given_theta
