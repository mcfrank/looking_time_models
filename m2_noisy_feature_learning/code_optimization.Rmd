    ...
    # initialize the thing 
     ...
     for (i in 1:n_possible_combination){
      ...
      for (s in 1:n_possible_combination){
        ...
        #creature_noisy_post_pred(1, all_possible_outcomes, posterior)
        #noisy_post_pred(prev_posterior_list[[ceiling(s/2)]]$theta, prev_posterior_list[[ceiling(s/2)]]$epsilon, prev_posterior_list[[ceiling(s/2)]]$posterior, TRUE)
 ...
 # figure out all the possible combinations 
  ...
  eig_list <- lapply(list_all_possible_combination, 
          function(df){
            ...
            creature_kl * creature_post_pred
            #c(creature_kl, creature_post_pred)
          })
...
# CURRENTLY BUGGGY : 3 FEATURES
observations = multifeature_stimuli %>% select(V1, V2)
...
old_post_pred <- get_all_possible_creature_pred(
...
bm_kl <- bench::mark(old_get_eig(observations), 
            new_get_eig(observations))

bm_kl %>% autoplot() + labs(title = "1 feature")
