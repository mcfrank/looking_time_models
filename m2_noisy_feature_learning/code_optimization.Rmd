...
x <- compositions(n = 2, m = 2, include.zero = F)
...
test_kl <- function(all_possible_creatures, observations, kls, plot = TRUE){
 ...
 # figure out all the possible combinations 
  ...
  # use compositions: 
   #compositions(7, 2, include.zero = TRUE)
  

  
  possible_options <- mapply(function(x, y){c(x, y)}, 
                             all_possible_combinations$past_observations,
                             all_possible_combinations$hypothetical_observation) %>% 
                             as.data.frame() %>% 
                             as.list()
