...
epsilons <- seq(0.1, .5, .2)
 ...
 old_posterior_after_one_udpate <- grid_apprxoimation_with_observation(
 ...
 old_posterior_after_os <- grid_apprxoimation_with_observation(
...
first_update <- faster_grid_apprxoimation_with_observation(
  timepoint = 1,
  noisy_observation = filter(os, t == 1), # i doubt this is a good idea
...
second_update <- faster_grid_apprxoimation_with_observation(
  timepoint = 2,
  noisy_observation = filter(os, t == 2), # i doubt this is a good idea
...
old_posterior_after_one_udpate
first_update


old_posterior_after_os
second_update
```
...
new_posterior <- faster_grid_apprxoimation_with_observation(
  timepoint=1,
  noisy_observation = os %>% filter(t==1),
  last_update_posterior_df = NULL, 
  track_epsilon = TRUE, 
  grid_theta = thetas, 
                          grid_epsilon = epsilons, 
...
posterior_after_o2 <- faster_grid_apprxoimation_with_observation(
  timepoint=2,
  noisy_observation = os %>% filter(t==2),
  last_update_posterior_df = new_posterior, 
  track_epsilon = TRUE, 
  grid_theta = thetas, 
                          grid_epsilon = epsilons, 
                          alpha_prior = alpha_prior, 
...
ptm <- proc.time()
 ...
 posterior_after_os <- grid_apprxoimation_with_observation(
                          ...
                          grid_epsilon = expensive_epsilon, 
                          alpha_prior = alpha_prior, 
                          beta_prior = beta_prior,
                          alpha_epsilon = alpha_epsilon, 
                          beta_epsilon = beta_epsilon
