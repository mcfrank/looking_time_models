...
# returns a dataframe of log probabilities

get_df_lp_z_given_theta <- function(t, # timestep
                                    f, # feature
                                    df_lp_y_given_theta, # cached likelihoods
                                    ll_df_z_given_theta, # likelihoods
                                    df_model, 
                                    current_observation) {
  
  # set up current variables
  current_df_lp_z_given_theta <- ll_df_z_given_theta[[t]][[f]]
  grid_epsilon <- unique(current_df_lp_z_given_theta$epsilon)
  grid_theta <- unique(current_df_lp_z_given_theta$theta)
  stimulus_idx <- df_model$stimulus_idx[t]
  ...
  df_lp_z_given_y = tibble(epsilon = grid_epsilon)
  
  # if this is the first observation, just evaluate this one, otherwise sum over all observations
  if (t == 1 || stimulus_idx != df_model$stimulus_idx[[t-1]]) {
    df_lp_z_given_y$lp_z_given_y_ONE =  lp_z_ij_given_y(zij = current_observation[[f]], yi = 1, epsilon = grid_epsilon)
    df_lp_z_given_y$lp_z_given_y_ZERO = lp_z_ij_given_y(zij = current_observation[[f]], yi = 0, epsilon = grid_epsilon)
  } else {
...
get_df_post <- function(z_given_theta, df_lp_theta_epsilon, df_post) {
  
  df_post$unnormalized_log_posterior <- z_given_theta$lp_z_given_theta + 
    df_lp_theta_epsilon$lp_theta + 
    df_lp_theta_epsilon$lp_epsilon
  df_post$log_posterior <- df_post$unnormalized_log_posterior - matrixStats::logSumExp(df_post$unnormalized_log_posterior)
  df_post$posterior <- exp(df_post$log_posterior)
...
# 
# # sub function updating likelihood
# # returns a dataframe of log probabilities
# 
